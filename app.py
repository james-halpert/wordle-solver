...
def wordle_solver():
    ...
    if request.method == "POST":
        # Remove non-alpha from grey
        grey_letters = ''.join(char for char in grey_letters if char.isalpha())

        # Prevent greys from nuking known-present letters (basic sanity fix)
        present = set([c for c in green_letters + yellow_letters if c])
        grey_letters = ''.join([c for c in grey_letters if c not in present])
